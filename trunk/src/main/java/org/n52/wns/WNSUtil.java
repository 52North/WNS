...
public class WNSUtil {
	...
	private static Logger log = LoggerFactory.getLogger(WNSUtil.class);
	...
	public static void sendResponse(XmlTokenSource xts, String schemaLocation,
		...
		try {
			...
			log.error("Could not send response: " + e.toString());
			...
			if (out != null) {
				try {
					...
					log.error("Cannot close response output stream: "
							+ e.toString());
