...
import org.n52.wns.communication.PhoneNumber;
...
/**
 * Specific SMSHandler for the Ecall-Account http://www.ecall.com Ecall uses an
 * email-to-sms/speech/fax mechanism. Therefore SMS are sent as emails.
 * 
 * @author Dennis Dahlmann
 * @version 2.0
 */
public class SMSHandlerEcall implements SMSHandler {

	private static Logger log = LoggerFactory.getLogger(SMSHandlerEcall.class);
	...
	/**
	 * SMSHandlerEcall will get the systems default mail handler required for
	 * sending mails.
	 * 
	 * @throws WNSException
	 */
	public SMSHandlerEcall() throws WNSException {
		log.debug("Init SMSHandlerEcall");
		// loads the MailHandler used to send mails to ecall
...
//				+ dnd.getDoNotification().getShortMessage();
		for (int i = 0; i <= targets.length - 1; i++) {
			...
			String to = PhoneNumber.convertToInternationalFormat(msisdn) + "@"
					+ this.mailtoAdress;
