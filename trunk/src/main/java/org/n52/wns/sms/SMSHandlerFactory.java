...
/**
 * Factory to load the SMSHandler classes
 * 
...
public class SMSHandlerFactory {
	...
	private static Logger log = LoggerFactory.getLogger(SMSHandlerFactory.class);

	public static SMSHandler getInstance(
			WNSConfigDocument.WNSConfig.RegisteredHandlers.SMSHandler handler,
			String wns) throws WNSServiceException {
		log.debug("getInstance request");
		SMSHandler sms;

		try {
			sms = (SMSHandler) Class.forName(handler.getProvider())
					.newInstance();
			sms.setProperties(handler, wns);
			return sms;
		} catch (Exception e) {
