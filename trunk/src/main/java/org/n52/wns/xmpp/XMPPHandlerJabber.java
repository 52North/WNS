...
public class XMPPHandlerJabber implements org.n52.wns.xmpp.XMPPHandler {
	...
	private static Logger log = LoggerFactory.getLogger(XMPPHandlerJabber.class);
	...
	private void setup() throws WNSException {
		try {
			...
			try {
				...
				log.error("Error duriong XMPP connection establishment: "
						+ e1.toString());
	...
	private void login() throws WNSException {
		try {
			...
			log.error("Login to XMPP server failed: " + e.toString());
	...
	private void sendMessage(String recipient, String message)
		...
		try {
			...
			log.error("Message sending failed, recipient: " + recipient + " : "
					+ e1.toString());
