...
import org.x52North.wns.WNSUserDocument.WNSUser.SingleUser.User.Channel;
...
public class WNSUserHandler {
	...
	private static Logger log = Logger
			.getLogger(WNSUserHandler.class.getName());
	...
	private WNSUserHandler() throws WNSServiceException {
		...
		try {
			...
			log.fatal("Error during init USER DAO " + e.toString());
		...
		try {
			...
			log.fatal("Error during getting WNSUserDocument: " + e.toString());
	...
	public synchronized UpdateSingleUserRegistrationResponseDocument updateSingleUser(
		...
		// Update Name
		if (usurd.getUpdateSingleUserRegistration().isSetUpdateName()) {
			log.debug("Trying to update the name");
			if (this.singleHashthable.containsKey(new Long(usurd
					.getUpdateSingleUserRegistration().getUserID()))) {
				WNSUser wnsUser = this.singleHashthable.get(new Long(usurd
						.getUpdateSingleUserRegistration().getUserID()));
				wnsUser.setName(usurd.getUpdateSingleUserRegistration()
						.getUpdateName());
				WNSUserDocument.WNSUser user = this.userdoc.getWNSUser();
				org.x52North.wns.WNSUserDocument.WNSUser.SingleUser single = user
						.getSingleUser();
	...
	public synchronized UpdateMultiUserRegistrationResponseDocument updateMultiUser(
		...
		if (this.multiUserHashtable.containsKey(new Long(umurd
			...
			if (umurd.getUpdateMultiUserRegistration().isSetAddUser()) {
				...
				for (int i = 0; i < umurd.getUpdateMultiUserRegistration()
					...
					if (this.multiUserHashtable.containsKey(new Long(umurd
						...
						if (doubleUsers.contains(umurd
								.getUpdateMultiUserRegistration().getAddUser()
								.getIDArray(i))) {
							log.fatal("Circular dependency found in user: "
									+ umurd.getUpdateMultiUserRegistration()
											.getAddUser().getIDArray(i));
				...
				org.x52North.wns.WNSUserDocument.WNSUser.MultiUser multi = user
						.getMultiUser();
	...
	public synchronized UnregisterResponseDocument unregister(
		...
		if (this.singleHashthable.containsKey(new Long(ud.getUnregister()
				.getID()))) {
			org.x52North.wns.WNSUserDocument.WNSUser.SingleUser single = user
					.getSingleUser();
			for (int i = 0; i <= single.sizeOfUserArray() - 1; i++) {
				if (single.getUserArray(i).getID().equalsIgnoreCase(
						ud.getUnregister().getID())) {
					single.removeUser(i);
					this.singleHashthable.remove(new Long(ud.getUnregister()
							.getID()));
					this.checkMultiUser(ud.getUnregister().getID());
				}
			}

		} else {
			if (this.multiUserHashtable.containsKey(new Long(ud.getUnregister()
					.getID()))) {
				org.x52North.wns.WNSUserDocument.WNSUser.MultiUser multi = user
						.getMultiUser();
				...
				service.addCodedException(ExceptionCode.UnknownUserID, ud
						.getUnregister().getID());
				log.fatal("UNKNOWN_ID: " + ud.getUnregister().getID());
				throw service;
	...
	private void checkMultiUser(String id) throws WNSServiceException {
		...
		org.x52North.wns.WNSUserDocument.WNSUser.MultiUser multi = user
				.getMultiUser();
		org.x52North.wns.WNSUserDocument.WNSUser.MultiUser.User multiuser;
	...
	public synchronized Channel[] getMessageChannels(String userID)
		...
		if (this.singleHashthable.containsKey(new Long(userID))) {
			...
			if (this.multiUserHashtable.containsKey(new Long(userID))) {
				org.x52North.wns.WNSUserDocument.WNSUser.SingleUser.User user = org.x52North.wns.WNSUserDocument.WNSUser.SingleUser.User.Factory
						.newInstance();
