...
/**
 * Singleton holding information and objects required by other components of 
 * the WNS.
 * 
...
public class WNSInitParamContainer {
	...
	private static Logger log = Logger.getLogger(WNSInitParamContainer.class
			.getName());
   ...
   private WNSDatabaseThread thread = null;
	...
	private WNSInitParamContainer(WNSConfig wnsc) throws WNSException {
      ...
      try {
         ...
         String driver = wnsc.getServiceProperties()
               .getDAOFactoryDriver();
         Class c = Class.forName(driver);
         this.daoFactory = (DAOFactory) c.newInstance();
         this.daoFactory.configure(wnsc.getServiceProperties()
		...
		if (wnsc.getServiceProperties().getExceptionLevel().equals(
				WNSConfig.ServiceProperties.ExceptionLevel.PLAIN_EXCEPTIONS)) {
			this.exceptionLevel = ExceptionLevel.PlainExceptions;
		} else {
			this.exceptionLevel = ExceptionLevel.DetailedExceptions;
		}

		this.wnsSchemaLocation = wnsc.getServiceProperties()
				.getWNSSchemaLocation();
		this.ogcExcSchemaLocation = wnsc.getServiceProperties()
				.getOWSSchemaLocation();
	 ...
	 */
	public static synchronized WNSInitParamContainer createInstance(
	      WNSConfigDocument wnscd) throws WNSException {
		...
		if (instance == null) {
	      ...
	      instance.thread = new WNSDatabaseThread(
	            wnscd.getWNSConfig().getServiceProperties().getDataBaseProperties().getCheckDBDuration(),
