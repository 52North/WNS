...
public class WNSMessageHandler implements WNSMessageHandlerI {
	...
	private static Logger log = Logger.getLogger(WNSMessageHandler.class
			.getName());
	...
	public WNSMessageHandler(WNSUserHandler userHandler)
		...
		if (wnsConfig.getRegisteredHandlers().isSetEMailHandler()) {
			try {
				...
				log.fatal("Error during MailHandlerFactory getInstance: "
						+ e.toString());
			}
		}
		if (wnsConfig.getRegisteredHandlers().isSetSMSHandler()) {
			try {
				...
				log.fatal("Error during SMSHandlerFactory getInstance: "
						+ e.toString());
			}
		}
		if (wnsConfig.getRegisteredHandlers().isSetPhoneHandler()) {
			try {
				...
				log.fatal("Error during PhoneHandlerFactory getInstance: "
						+ e.toString());
			}
		}
		if (wnsConfig.getRegisteredHandlers().isSetFaxHandler()) {
			try {
				...
				log.fatal("Error during FaxHandlerFactory getInstance: "
						+ e.toString());
			}
		}
		if (wnsConfig.getRegisteredHandlers().isSetXMPPHandler()) {
			try {
				...
				log.fatal("Error during XMPPHandlerFactory getInstance: "
						+ e.toString());
			}
		...
		try {
			...
			log.fatal("Error during WNSWebServiceHandler getInstance: "
					+ e.toString());
		}
	...
	public synchronized DoNotificationResponseDocument doNotification(
		...
		DoNotificationResponseDocument response = DoNotificationResponseDocument.Factory
				.newInstance();
		org.x52North.wns.WNSUserDocument.WNSUser.SingleUser.User.Channel[] channel = null;
		log.debug("Trying to doNotification");
		try {
			channel = this.userHandler.getMessageChannels(dnd
					.getDoNotification().getUserID());
		} catch (WNSException e) {
			log.fatal("UnknownUserID: " + dnd.getDoNotification().getUserID());
			WNSServiceException s = new WNSServiceException(
					WNSInitParamContainer.getInstance().getExceptionLevel());
			s.addCodedException(
		...
		for (int i = 0; i < channel.length; i++) {
			...
			case org.x52North.wns.WNSUserDocument.WNSUser.SingleUser.User.Channel.Protocol.INT_EMAIL:
				...
				break;
			case org.x52North.wns.WNSUserDocument.WNSUser.SingleUser.User.Channel.Protocol.INT_XMPP:
				...
				break;
			case org.x52North.wns.WNSUserDocument.WNSUser.SingleUser.User.Channel.Protocol.INT_SMS:
				...
				break;
			case org.x52North.wns.WNSUserDocument.WNSUser.SingleUser.User.Channel.Protocol.INT_HTTP:
				...
				break;
			case org.x52North.wns.WNSUserDocument.WNSUser.SingleUser.User.Channel.Protocol.INT_PHONE:
				...
				break;
			case org.x52North.wns.WNSUserDocument.WNSUser.SingleUser.User.Channel.Protocol.INT_FAX:
