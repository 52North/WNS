...
public class WNSRequestHandler {
	...
	private static Logger log = Logger.getLogger(WNSRequestHandler.class
			.getName());
	...
	public void handleGetRequest(HttpServletRequest request,
		...
		while (params.hasMoreElements()) {
			...
			if (p.equalsIgnoreCase(WNSConstants.REQUEST)) {
				...
				if (paramReq.equalsIgnoreCase(WNSConstants.LISTWNSUSER)) {
               ...
               .getInstance();
				...
				if (paramReq.equalsIgnoreCase(WNSConstants.GETWSDL)) {
					...
					File wsdl = new File(WNSInitParamContainer.getInstance()
							.getWSDLDocumentPath());
					FileReader reader;
					try {
					...
					if (sections != null) {
						...
						try {
							...
							log
									.fatal("Could not handle GetCapabilities request: "
											+ e.toString());
							throw new WNSException(
									"Could not handle GetCapabilities request.",
									e);
					...
					log.fatal("OperationNotSupported: " + paramReq);
					se
							.addCodedException(
		...
		if (!requestFound) {
			log.fatal("MissingParameterValue: REQUEST");
	...
	private void sendCompleteCapabilities(HttpServletResponse response)
		...
		try {
			...
			log.fatal("XmlException occured: " + e.toString());
			WNSServiceException se = new WNSServiceException(this.excLevel);
			se
					.addCodedException(
	...
	public void handlePostRequest(HttpServletRequest request,
		...
		try {
			...
			try {
				...
				log.fatal("IOException occured: " + e.toString());
				WNSServiceException se = new WNSServiceException(this.excLevel);
				se.addCodedException(
						WNSServiceException.ExceptionCode.NoApplicableCode,
						null, e);
			...
			if (type == GetCapabilitiesDocument.type) {
				...
				if (!isValid) {
					...
					GetMessageResponseDocument gmrd = messageHandler
							.getMessage(gmd);
					WNSUtil
							.sendResponse(gmrd, this.wnsSchemaLocation,
									response);
				}

			} else {
				// unknown request
				log.fatal("OperationNotSupported from: "
						+ request.getRemoteAddr());
				WNSServiceException se = new WNSServiceException(this.excLevel);
				se
						.addCodedException(
								WNSServiceException.ExceptionCode.OperationNotSupported,
								null, "The operation you requested ("
