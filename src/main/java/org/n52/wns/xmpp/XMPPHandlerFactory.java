...
/**
 * A factory that manages the classes to load as a XMPPHandler This is a
 * Singleton pattern! Use the getInstance method!
 * 
 * @author Dennis Dahlmann
 * @version 2.0
 */

public class XMPPHandlerFactory {
	private static Logger log = LoggerFactory.getLogger(XMPPHandlerFactory.class);

	/**
	 * The Singleton 'consturctor'
	 * 
	 * @param xmppHandler
	 *            The XMPPHandler elemet
	 * @return The XMPPHandler
	...
	public static XMPPHandler getInstance(
		...
		try {
			xmpp = (XMPPHandler) Class.forName(xmppHandler.getProvider())
					.newInstance();
			xmpp.setProperties(xmppHandler);
			return xmpp;
		} catch (Exception e) {
			log.error("Error during getInstance request: " + e.toString());
