...
public class ExistDatabaseMessageDAO implements MessageDAO {
   ...
   private static final String MESSAGE_DOCUMENT_PREFIX = "N";
   ...
   private static Logger log = LoggerFactory.getLogger(ExistDatabaseMessageDAO.class);
   ...
   public ExistDatabaseMessageDAO(DataBaseProperties properties)
      ...
      try {
         ...
         log.error("Error during init ExistDatabaseMessageDAO request: "
               + e.toString());
   ...
   public GetMessageResponseDocument getMessage(String mID) throws WNSException, WNSServiceException {
      ...
      try {
         ...
         if(res != null && mID.substring(0, 1).equalsIgnoreCase(MESSAGE_DOCUMENT_PREFIX)) {
               ...
               try {
                  ...
                  log.error("Exception while trying to retrieve message from database.",e);
   ...
   public void storeNotificationMessage(DoNotificationType message,
      ...
      try {
         document = (XMLResource) this.col.createResource(MESSAGE_DOCUMENT_PREFIX + messageID,
               "XMLResource");
         document.setContent(message.getMessage().xmlText());
         this.col.storeResource(document);
         this.col.close();

         String id = MESSAGE_DOCUMENT_PREFIX + messageID;
   ...
   private int loadMessages() throws WNSException {
      ...
      try {
         ...
         log.error("Cannot load stored message numbers: " + e.toString());
         WNSException we = new WNSException(
